...
class FileEncryptor(QWidget):
    ...
    def init_ui(self):
        '''Initialises the UI layout and widgets with styling'''
        self.setWindowTitle("File Encryption & Decryption Tool")
        self.setGeometry(100, 100, 450, 300)

        # Apply custom font & layout
    ...
    def _derive_key(self, password):
        ...
        # Use SHA-256 to get a 32-byte key, then base64 encode it for Fernet
        digest = hashlib.sha256(password.encode()).digest()
        return base64.urlsafe_b64encode(digest)

    def encrypt_file(self, file_path, password):
        """Encrypts the selected file using Fernet symmetric encryption."""
    ...
    def decrypt_file(self, file_path, password):
        """Decrypts the selected file using Fernet symmetric encryption."""
        key = self._derive_key(password)
        cipher = Fernet(key)
        try:
            with open(file_path, "rb") as file:
